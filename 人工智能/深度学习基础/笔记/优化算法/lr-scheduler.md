...
![svg](imgs/lr-scheduler_files/lr-scheduler_3_1.svg)
## 学习率调度器
...
```
![svg](imgs/lr-scheduler_files/lr-scheduler_9_0.svg)
...
![svg](imgs/lr-scheduler_files/lr-scheduler_11_1.svg)
...
为了防止学习率衰减到一个合理的下界之下，更新方程经常修改为$\eta_{t+1} \leftarrow \mathop{\mathrm{max}}(\eta_{\mathrm{min}}, \eta_t \cdot \alpha)$。
```python
...
```
![svg](imgs/lr-scheduler_files/lr-scheduler_13_0.svg)
接下来，我们将使用内置的调度器，但在这里仅解释它们的功能。
### 多因子调度器
训练深度网络的常见策略之一是保持学习率为一组分段的常量，并且不时地按给定的参数对学习率做乘法衰减。
具体地说，给定一组降低学习率的时间点，例如$s = \{5, 10, 20\}$，每当$t \in s$时，降低$\eta_{t+1} \leftarrow \eta_t \cdot \alpha$。
...
```
![svg](imgs/lr-scheduler_files/lr-scheduler_15_0.svg)
...
![svg](imgs/lr-scheduler_files/lr-scheduler_17_1.svg)
### 余弦调度器
...
d2l.plot(torch.arange(num_epochs), [scheduler(t) for t in range(num_epochs)])
```
![svg](imgs/lr-scheduler_files/lr-scheduler_19_0.svg)
...
![svg](imgs/lr-scheduler_files/lr-scheduler_21_1.svg)
### 预热
...
d2l.plot(torch.arange(num_epochs), [scheduler(t) for t in range(num_epochs)])
```
![svg](imgs/lr-scheduler_files/lr-scheduler_23_0.svg)
...
![svg](imgs/lr-scheduler_files/lr-scheduler_25_1.svg)
